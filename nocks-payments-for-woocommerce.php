...
function nocks_wc_plugin_activation_hook ()
{
    ...
    {
        $title = sprintf(
            __('Could not activate plugin %s', 'nocks-checkout-for-woocommerce'),
            'Nocks Payments for WooCommerce'
        );
        $message = ''
...
function nocks_wc_plugin_deactivated ()
{
    ...
    echo '<div class="error"><p>' . sprintf(__('%s deactivated because it depends on WooCommerce.', 'nocks-checkout-for-woocommerce'), Nocks_WC_Plugin::PLUGIN_TITLE) . '</p></div>';
}
