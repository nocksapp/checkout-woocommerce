...
class Nocks_WC_Helper_Status
{
    ...
    public function isCompatible() {
        ...
        if (!$this->hasCompatibleWooCommerceVersion()) {
            $this->errors[] = sprintf(/* translators: Placeholder 1: Plugin name, placeholder 2: required WooCommerce version, placeholder 3: used WooCommerce version */
                __('The %s plugin requires at least WooCommerce version %s, you are using version %s. Please update your WooCommerce plugin.', 'nocks-checkout-for-woocommerce'), Nocks_WC_Plugin::PLUGIN_TITLE, self::MIN_WOOCOMMERCE_VERSION, $this->getWooCommerceVersion());

            return $is_compatible = false;
        }

        if (!$this->isApiClientInstalled()) {
            $this->errors[] = __('Nocks API client not installed. Please make sure the plugin is installed correctly.', 'nocks-checkout-for-woocommerce');
            ...
            switch ($e->getCode()) {
                case Nocks_Exception_IncompatiblePlatform::INCOMPATIBLE_PHP_VERSION:
                    $error = sprintf(/* translators: Placeholder 1: Required PHP version, placeholder 2: current PHP version */
                        __('The client requires PHP version >= %s, you have %s.', 'nocks-checkout-for-woocommerce'), Nocks_WC_CompatibilityChecker::$MIN_PHP_VERSION, PHP_VERSION);
                    break;
                default:
                    $error = $e->getMessage();
                    break;
    ...
    public function getNocksMerchants() {
        ...
        // Try to load Nocks issuers
        $api_client->merchant_accounts;
        ...
        try {
            ...
            /**
             * @var $api_client Nocks_Checkout
             */
            return $api_client->getMerchants();
