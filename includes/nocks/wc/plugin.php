...
class Nocks_WC_Plugin
{
        ...
        foreach ($items as $item){
            ...
            if ($order->get_status() == Nocks_WC_Gateway_Abstract::STATUS_COMPLETED){
                ...
                /* translators: Placeholder 1: payment method title, placeholder 2: payment ID */
                    __('%s payment failed (%s).', 'nocks-checkout-for-woocommerce'),
    ...
    public static function addPluginActionLinks (array $links)
    {
        $action_links = array(
            ...
            '<a href="' . self::getSettingsHelper()->getGlobalSettingsUrl() . '">' . __('Nocks settings', 'nocks-checkout-for-woocommerce') . '</a>',
        );
        ...
        if (version_compare(self::getStatusHelper()->getWooCommerceVersion(), '2.2.0', ">="))
        {
            ...
            $action_links[] = '<a href="' . self::getSettingsHelper()->getLogsUrl() . '">' . __('Logs', 'nocks-checkout-for-woocommerce') . '</a>';
        }
    ...
    /**
     * @return Nocks_WC_Helper_Merchants
     */
    public static function getMerchantsHelper ()
    {
        static $merchants_helper;

        if (!$merchants_helper)
        {
            $merchants_helper = new Nocks_WC_Helper_Merchants();
        }

        return $merchants_helper;
    }
}
