...
abstract class Nocks_WC_Gateway_Abstract extends WC_Payment_Gateway
{
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'enabled' => array(
                'title'       => __('Enable/Disable', 'nocks-checkout-for-woocommerce'),
                'type'        => 'checkbox',
	...
	public function getIssuerIconUrl( $issuer_id ) {
		return Nocks_WC_Plugin::getPluginUrl( 'assets/images/' . $issuer_id . '.png' );
	}
    ...
    public function admin_options ()
    {
        ...
        {
            echo '<div class="inline error"><p><strong>' . __('Gateway Disabled', 'nocks-checkout-for-woocommerce') . '</strong>: '
    ...
    protected function isValidForUse()
    {
        ...
        if (!$this->isValidApiKeyProvided())
        {
            ...
            $this->errors[] = ($test_mode ? __('Test mode enabled.', 'nocks-checkout-for-woocommerce') . ' ' : '') . sprintf(
                /* translators: The surrounding %s's Will be replaced by a link to the global setting page */
                    __('No API key provided. Please %sset your Nocks API key%s first.', 'nocks-checkout-for-woocommerce'),
        ...
        if (!$this->isCurrencySupported())
        {
            ...
            /* translators: Placeholder 1: WooCommerce currency, placeholder 2: Supported Nocks currencies */
                __('Shop currency %s not supported by Nocks. Nocks only supports: %s.', 'nocks-checkout-for-woocommerce'),
     ...
     * @return null|string
     */
    protected function getUserNocksCustomerId($order, $test_mode)
    {
    ...
    public function webhookAction ()
    {
        ...
        else
        {
            ...
            /* translators: Placeholder 1: payment method title, placeholder 2: payment status, placeholder 3: payment ID */
                __('%s payment %s (%s).', 'nocks-checkout-for-woocommerce'),
                $this->method_title,
                $payment->status,
                $payment->id . ($payment->mode == 'test' ? (' - ' . __('test mode', 'nocks-checkout-for-woocommerce')) : '')
            ));
        }
    ...
    protected function onWebhookCompleted(WC_Order $order, $payment)
    {
        ...
        $order->add_order_note(sprintf(
        /* translators: Placeholder 1: payment method title, placeholder 2: payment ID */
            __('Order completed using %s payment (%s).', 'nocks-checkout-for-woocommerce'),
            $paymentMethodTitle,
            $payment->id . ($payment->mode == 'test' ? (' - ' . __('test mode', 'nocks-checkout-for-woocommerce')) : '')
        ));
    ...
    protected function onWebhookCancelled(WC_Order $order, Nocks_Transaction $transaction)
    {
        ...
        // User cancelled payment on Nocks or issuer page, add a cancel note.. do not cancel order.
        $order->add_order_note(sprintf(
        /* translators: Placeholder 1: payment method title, placeholder 2: payment ID */
            __('%s payment cancelled (%s).', 'nocks-checkout-for-woocommerce'),
            $paymentMethodTitle,
            $payment->id
        ));
    }
    ...
    protected function onWebhookExpired(WC_Order $order, Nocks_Object_Payment $payment)
    {
	    ...
	    if ( $nocks_payment_id != $payment->id) {
		    ...
		    $order->add_order_note(sprintf(
		    /* translators: Placeholder 1: payment method title, placeholder 2: payment ID */
			    __('%s payment expired (%s) but order not cancelled because of another pending payment (%s).', 'nocks-checkout-for-woocommerce'),
			    $paymentMethodTitle,
			    $payment->id . ($payment->mode == 'test' ? (' - ' . __('test mode', 'nocks-checkout-for-woocommerce')) : ''),
			    $nocks_payment_id
		    ));
        ...
        $order->add_order_note(sprintf(
        /* translators: Placeholder 1: payment method title, placeholder 2: payment ID */
            __('%s payment expired (%s).', 'nocks-checkout-for-woocommerce'),
            $paymentMethodTitle,
            $payment->id . ($payment->mode == 'test' ? (' - ' . __('test mode', 'nocks-checkout-for-woocommerce')) : '')
        ));
	...
	public function getReturnRedirectUrlForOrder( WC_Order $order ) {
		...
		if ( $this->orderNeedsPayment( $order ) ) {
			...
			if ( $hasCancelledNocksPayment ) {
				...
				Nocks_WC_Plugin::addNotice( __( 'You have cancelled your payment. Please complete your order with a different payment method.', 'nocks-checkout-for-woocommerce' ) );
    ...
    public function displayInstructions(WC_Order $order, $admin_instructions = false, $plain_text = false)
    {
        ...
        if (!empty($instructions))
        {
            ...
            {
                echo '<h2>' . __('Payment', 'nocks-checkout-for-woocommerce') . '</h2>';
    ...
    protected function getInstructions (WC_Order $order, $transaction, $admin_instructions, $plain_text)
    {
        ...
        if ($transaction->isOpen())
        {
            ...
            {
                // Message to admin
                return __('We have not received a definite payment status.', 'nocks-checkout-for-woocommerce');
            }
            else
            {
                // Message to customer
                return __('We have not received a definite payment status. You will receive an email as soon as we receive a confirmation of the bank/merchant.', 'nocks-checkout-for-woocommerce');
            }
        } elseif ($transaction->isCancelled())
        {
            ...
            {
                // Message to admin
                return __('Your payment has been cancelled.', 'nocks-checkout-for-woocommerce');
            }
            else
            {
                // Message to customer
        ...
        elseif ($transaction->isPaid())
        {
            ...
            /* translators: Placeholder 1: payment method */
                __('Payment completed with <strong>%s</strong>', 'nocks-checkout-for-woocommerce'),
                $this->get_title()
            );
        }
    ...
    {
        $issuer_id = Nocks_WC_Plugin::PLUGIN_ID . '_issuer_' . $this->id;
    ...
    protected function isValidApiKeyProvided ()
    {
        ...
        $merchant_key = $settings->getMerchantAccount();
        return !empty($api_key) && !empty($merchant_key);
    }
